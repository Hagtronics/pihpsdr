...
static gboolean hw_agc_state = FALSE;
...
void hw_agc_menu(GtkWidget *parent) {
  dialog = gtk_dialog_new();
  gtk_window_set_transient_for(GTK_WINDOW(dialog), GTK_WINDOW(parent));
  char title[64];

  snprintf(title, sizeof(title), "piHPSDR - HW AGC");
  ...
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (hw_agc_cb), hw_agc_state);
